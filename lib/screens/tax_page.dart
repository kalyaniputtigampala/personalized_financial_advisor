...
class _TaxPageState extends State<TaxPage> {
            ...
            child: Column(
              ...
              children: [
                ...
                _buildSummarySection(
                  ...
                  children: [
                    _buildResultRow('Gross Annual Income (Non-Agricultural)', grossIncome, icon: Icons.attach_money),
                    if (agriculturalIncome > 0) ...[
                      _buildResultRow('Agricultural Income (Exempt)', agriculturalIncome,
                          isExempt: true, icon: Icons.eco),
                ...
                _buildSummarySection(
                  ...
                  children: [
                    _buildResultRow('Total Deductions', totalDeductions, icon: Icons.arrow_downward),
                    _buildResultRow('Taxable Income', taxableIncome, icon: Icons.work),
                    _buildResultRow('Tax Calculated', calculatedTax, icon: Icons.calculate),
                    _buildResultRow('Effective Tax Rate', double.parse(getEffectiveTaxRate()),
                        isPercentage: true, icon: Icons.percent),
                    _buildResultRow('Tax with Health & Education Cess (4%)', finalTax, icon: Icons.local_hospital),
                  ],
                ),
                ...
                _buildSummarySection(
                  ...
                  icon: Icons.payment,
                  children: [
                    _buildResultRow('Tax Already Paid', taxPaid, icon: Icons.check_circle),
                    const Divider(thickness: 2, color: primaryColor),
                    _buildResultRow(
                      refundOrPay < 0 ? 'Refund Due' : 'Additional Tax to Pay',
                      refundOrPay.abs(),
                      isHighlighted: true,
                      icon: refundOrPay < 0 ? Icons.money : Icons.payment,
                    ),
                  ],
                ),
  ...
  Widget _buildSummarySection({required String title, required List<Widget> children, IconData? icon}) {
    return Container(
      margin: const EdgeInsets.only(bottom: 20),
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
  ...
  Widget _buildInfoBox({required String title, required Color color, required List<String> items, IconData? icon}) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
  ...
  List<Widget> getTaxOptimizationTips() {
    ...
    return tipsList.map((tip) => _buildTaxTipCard(
      ...
      title: tip['title'] ?? 'Tax Tip',
      description: tip['description'] ?? '',
    )).toList();
  }

  Widget _buildTaxTipCard({required IconData icon, required String title, required String description}) {
    return Container(
        ...
        children: [
              ...
              children: [
                if (icon != null) ...[
                  Icon(
                    icon,
                    size: 16,
                    color: isExempt ? Colors.green[700] : primaryColor,
                  ),
                  const SizedBox(width: 8),
                ],
                Expanded(
          ...
          Expanded(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: isHighlighted ? primaryColor.withOpacity(0.1) :
              ...
              child: Text(
                ...
                style: TextStyle(
                  ...
                  color: isHighlighted ? primaryColor :
